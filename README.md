...
- **TOP**(`/items`) - ファイル管理のメイン画面。ファイル一覧(カード/テーブル切替、既定はテーブル)・検索/絞り込み・アップロード・詳細表示/編集/削除をすべてここで行う
  - Google Driveライクなアップロード: ページ上部の「アップロード」ボタン、またはページ全体へのドラッグ&ドロップでファイルをすぐにアップロード・仮登録し、詳細は右側のサイドバーで後から入力する
  - カード/行をクリックすると右側のサイドバーにその場で詳細を表示(ページ遷移なしのマスタ・ディテール表示。URLは連動して更新されるので直接リンク/リロードも可能)。「編集」をクリックするとサイドバー内でそのまま編集フォームに切り替わる(別ページには遷移しない)。ダウンロード・削除もここから
  - 複数選択はCtrl/Cmd+クリック(個別トグル)・Shift+クリック(範囲選択)・マウスドラッグ(矩形選択)の3通り(チェックボックスは使わない)。選択すると一括操作バーが表示される
  - **アバターリスト**(`/avatars`) - 登録済みのアバター(=アバター本体として登録したアイテム)をサムネイル付きで一覧表示。クリックでそのアバターの本体アイテムへ、対応商品数のバッジからそのアバターに絞り込んだTOP画面へ
  - **ショップリスト**(`/shops`) - 商品を持っているショップを一覧表示。ショップURLがある場合は「Boothから取得」でアイコン・説明文を自動取得できる
- **設定**(`/settings`) - Google Drive連携・**Driveと同期**・整合性チェック・**ショップ管理**・OAuthクライアント・ログインパスワード・動作設定をまとめた画面

## 主な機能

- サイドバーの編集フォームは8項目に絞ってある: BoothURL・サムネイル・タイトル・ショップ名・説明・対応アバター・タグ・メモ。価格・購入日・ステータス・お気に入り・ライセンスはここでは編集せず(値は保持され、詳細表示と一括編集からは引き続き参照・変更できる)
- BoothURLを貼り付けると、サムネイル・タイトル・ショップ名・説明を自動取得(取得後も修正可能。失敗時は手動入力にフォールバック)。対応アバターは登録済みのものの中からBOOTHの文面に一致するものを自動でチェック
- **対応アバターはアップロード済みのアイテムを本体として登録する方式**: アイテムの編集画面で「このアイテムをアバターとして登録する」を有効にし、アバター名・メモを設定する(自由入力のタグではない)。他のアイテムの「対応アバター」はこうして登録済みのアバターの中からチェックボックスで選ぶ
- タグは自由入力+自動マスタ登録
- キーワード検索・タグ/アバター/ショップ/ステータス絞り込み(カード/テーブル表示切替)。検索・一括編集の候補入力はネイティブdatalistではなく独自スタイルのコンボボックス
- 複数選択による一括操作(一括削除・ステータス/タグ/対応アバター/お気に入りの一括編集)
...
- **Drive上で直接行った変更の取り込み**(設定画面の「Driveと同期」): Drive上のフォルダ構成は `VRC-ContentManager/_db`(DBスナップショット)・`upload`(Drive側で手動追加したファイルの受付フォルダ)・`file`(アプリが管理する実ファイルのフラットな格納場所)の3つ。同期実行時に、(1) 手動で削除・移動して見つからなくなったファイルの参照をDB側から削除(商品自体やタグ等のメタデータは保持)、(2) `upload`フォルダに直接追加されたファイルを新しい商品として取り込み(ショップは「未設定」で登録、詳細はあとから編集)、(3) 旧フォルダ構成に残っている既存ファイルを`file`フォルダへ移行、を行う。フォルダ自体が削除されていた場合も次回アクセス時に自動再作成される。ユーザー操作で明示的に実行する方式(バックグラウンドでの自動実行はしない)。同期・整合性チェック・アップロードの各操作は開始時点で進行状況インジケーターを表示する。
- **同名ファイルの重複検出**: アップロード時、既存の商品と同じファイル名だった場合はその場で統合するか確認するトースト通知が出る(統合してもアップロード自体は失敗しない — 別々のまま登録も選べる)。それより前に登録済みの重複や確認をスキップしたものは、設定画面の「重複ファイルの検出」からまとめて検出・統合できる。

現時点で未実装(既知の今後の課題): 同一販売URL・別対応アバターの商品をまとめて表示するグルーピング。

## UI

サーバーサイドレンダリング(Jinja2 + HTMX)。ヘッダー(サービス名・表示モード切替・設定・ユーザーメニュー)+ 左サイドバー(一覧/アバターリスト/ショップリスト)+ コンテンツの構成。見た目は細めのフォント(Noto Sans JP Light)と半透明・ぼかし(glassmorphism)を基調にした、Google Fonts + Google Icons(Material Symbols Outlined)使用のUI。パフォーマンスのため、繰り返し表示されるカード/行自体・共通コンポーネント(ボタン・入力欄・タグchip等)にはぼかしを掛けず(既にぼかし済みのガラスパネルの内側でさらにぼかすのは見た目上の効果がない一方でコストだけ発生するため)、ヘッダーやサイドバー詳細パネルなど重なりが実際に発生する単一要素にのみぼかしを適用している。長い一覧は`content-visibility`で画面外の描画をスキップする。

- CSSフレームワークは [Tailwind CSS](https://tailwindcss.com/)。実行時JITコンパイル(Play CDN)によるスクロール等のもたつきを避けるため、開発時に `npm run build:css` で `app/static/css/tailwind.css` を事前生成してコミットする方式(本番実行時にNode.jsは不要)。**テンプレートのTailwindクラスを変更したら、コミット前に `npm install && npm run build:css` を実行して生成物を更新すること**(`npm run watch:css` で監視ビルドも可能)。共通コンポーネント(ボタン・フォーム項目・カード・アラート等)は `app/templates/_macros.html` のJinjaマクロにまとめて重複を防いでいる。

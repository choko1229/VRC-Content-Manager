...
サーバーサイドレンダリング(Jinja2 + HTMX、ビルドステップなし)。ヘッダー(サービス名・表示モード切替・設定・ユーザーメニュー)+ 左サイドバー(一覧/アバターリスト/ショップリスト)+ コンテンツの構成。見た目は細めのフォント(Noto Sans JP Light)と半透明・ぼかし(glassmorphism)を基調にした、Google Fonts + Google Icons(Material Symbols Outlined)使用のUI。パフォーマンスのため、繰り返し表示されるカード/行自体にはぼかしを掛けず(背景が単色のため視覚効果がない一方でコストだけ発生するため)、ヘッダーやサイドバー詳細パネルなど重なりが実際に発生する単一要素にのみぼかしを適用している。長い一覧は`content-visibility`で画面外の描画をスキップする。

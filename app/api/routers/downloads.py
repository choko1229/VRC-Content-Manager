...
def download_item_file(item_id: int, db: Session = Depends(get_db)):
    ...
    drive_client = None
    if primary.synced_at is not None:
        try:
    ...
    try:
        path = file_content_service.resolve_local_path(primary, drive_client)
    ...
    logger.info("item id=%s downloaded (item_file id=%s)", item_id, primary.id)
    return FileResponse(
        path=path,
